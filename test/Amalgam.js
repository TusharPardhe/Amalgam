...
contract('Amalgam', function(accounts) {
  var tokenInstance;
  ...
  it('initializes the contract with the correct values', function() {
    return Amalgam.deployed().then(function(instance) {
      ...
      assert.equal(name, 'Amalgam', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'XHg', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'Amalgam v1.0', 'has the correct standard');
    });
  })
  ...
  it('approves tokens for delegated transfer', function(){
    return Amalgam.deployed().then(function(instance){
      ...
      assert.equal(success, true, 'it returns true');
      return tokenInstance.approve(accounts[1],100); // In call we don't modify data but it this we trigger the event
    }).then(function(receipt){
      assert.equal(receipt.logs.length, 1, 'triggers one event');
  ...
  it('handles delegated transfers', function(){
    return Amalgam.deployed().then(function(instance){
      ...
      // Try trasferring something larger than sender's balance
      return tokenInstance.transferFrom(fromAccount, toAccount, 9999, { from: spendingAccount});
    }).then(assert.fail).catch(function(error){
      assert(error.message.indexOf('revert') >= 0, 'cannot transfer value larger than the balance');
      // Try tranferring something larger than the approval account
